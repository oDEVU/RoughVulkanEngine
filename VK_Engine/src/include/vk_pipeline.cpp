...
namespace rvk {
    ...
    VK_pipeline::VK_pipeline(
                ...
                : vk_device{device} {
        create_gfx_pipeline(vert_filepath, frag_filepath, config_info);
    }

    VK_pipeline::~VK_pipeline() {
        vkDestroyShaderModule(vk_device.device(), vert_shader_module, nullptr);
        vkDestroyShaderModule(vk_device.device(), frag_shader_module, nullptr);
        vkDestroyPipeline(vk_device.device(), gfx_pipeline, nullptr);
    }

    std::vector<char> VK_pipeline::read_file(const std::string& filepath) {
    ...
    void VK_pipeline::create_gfx_pipeline(
                const std::string& vert_filepath,
                const std::string& frag_filepath,
                const pipeline_config_info& config_info) {
        ...
        auto& binding_descriptions = config_info.bindingDesc;    
        auto& attribute_descriptions = config_info.attributeDesc; 
        ...
        if(vkCreateGraphicsPipelines(vk_device.device(), VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &gfx_pipeline) != VK_SUCCESS){
            throw std::runtime_error("Graphics pipeline failed to create");
        }
    }

    void VK_pipeline::create_shader_module(
                ...
                VkShaderModule* shader_module) {
                    ...
                    if (vkCreateShaderModule(vk_device.device(), &create_info, nullptr, shader_module) != VK_SUCCESS){
                        throw std::runtime_error("failed to create shader module");
                    }
    }

    void VK_pipeline::bind(VkCommandBuffer command_buffer) {
        vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gfx_pipeline);
    }

    void VK_pipeline::default_pipeline_config_info(pipeline_config_info& config_info) {
        ...
        */
        ...
        config_info.bindingDesc = VK_model::Vertex::get_binding_descriptions();
        config_info.attributeDesc = VK_model::Vertex::get_attribute_descriptions();
    }
